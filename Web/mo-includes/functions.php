...
function mo_date($time = NULL)
{
	...
	if ( $t == 0 )
		$text = '刚刚';
	elseif ( $t< 60 )
		$text = $t. '秒前'; // Less than a minute
	elseif ( $t< 60 * 60 )
		$text = floor( $t/ 60 ) . '分钟前'; // Less than an hour
	elseif ( $t< 60 * 60 * 24 )
		$text = floor( $t/ ( 60 * 60 ) ) . '小时前'; // Less than an day
	elseif ( $t< 60 * 60 * 24 * 3 )
		$text = floor( $time / ( 60 * 60 * 24 ) ) == 1 ? '昨天 '. date( 'H:i', $time ) :
						'前天 '. date( 'H:i', $time ) ; // Less than 3 days
	elseif ( $t< 60 * 60 * 24 * 30 )
		$text = date( 'm月d日 H:i', $time ); // Less than a mouth
	elseif ( $t< 60 * 60 * 24 * 365 )
		$text = date( 'm月d日', $time ); // Less than a year
	else
		$text = date( 'Y年m月d日', $time ); // More than a year
	return $text;
}
