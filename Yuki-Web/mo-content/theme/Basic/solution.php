<?php
    global $user_logged, $mo_solution;
    if (isset($mo_request[1])    && strlen($mo_request[1]) == 24) {
        $sid = $mo_request[1];
        if (!mo_load_solution($sid) || mo_get_solution('uid') != $user_logged) {
        ...
        $solution_list = mo_load_solutions();
        if ($solution_list) {
            ...
            foreach ($solution_list as $solution) {
                $sid = (string)$solution['_id'];
                mo_load_solution($sid);
                ...
                if ($suid == $user_logged) {
                    echo '<td><a href="/?r=solution/'.$solution['_id'].'">详情</a></td>';
                } else {
                    echo '<td></td>';
                }
                echo '</tr>';
