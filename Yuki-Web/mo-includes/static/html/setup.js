function check_info() {
	item=new Array("mongodb_host","mongodb_port","mongodb_user","mongodb_pwd","redis_host","redis_port","redis_pwd","admin_name","admin_pwd");
	url="setup.php?action=check&mongodb_host="+$("#mongodb_host").val()+"&mongodb_port="+$("#mongodb_port").val()+
			...
			"&redis_port="+$("#redis_port").val()+"&redis_pwd="+btoa($("#redis_pwd").val())+
			"&admin_name="+btoa($("#admin_name").val())+"&admin_pwd="+btoa($("#admin_pwd").val());
	req = $.ajax({
    type:'get',
    url:url,
